    ...
    def parse(self):
        ...
        if self.rorg == RORG.VLD:
            if len(self._bit_data) == 40:
                self.cmd =  enocean.utils.from_bitarray(self._bit_data[37:40])
            elif len(self._bit_data) == 32:
                self.cmd =  enocean.utils.from_bitarray(self._bit_data[29:32])
            else:
                self.cmd =  enocean.utils.from_bitarray(self._bit_data[5:8])
